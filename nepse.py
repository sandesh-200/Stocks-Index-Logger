    ...
    try:
        while True:
            ...
            if previous_data is not None and current_data != previous_data:
                speak_index(f"Change in Stock Index Detected by {per_change} percentage which now becomes {current_data} which means the market has been {'decreased' if per_change<0 else 'increased'}")
